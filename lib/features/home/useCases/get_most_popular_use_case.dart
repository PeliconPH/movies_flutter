...
abstract class GetMostPopularMoviesUseCaseProtocol {
  void execute({Success? success, Failure? failure});
}

class GetMostPopularMoviesUseCase extends GetMostPopularMoviesUseCaseProtocol {
  final MoviesRoutesProtocol routes;

  GetMostPopularMoviesUseCase({required this.routes});

  @override
  void execute({Success? success, Failure? failure}) {
    routes.getMostPopularMovies(
      success: (response) {
        try {
